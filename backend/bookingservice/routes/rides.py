...
from schemas.rides import RideCreate, RideUpdate, RideResponse, RideStatus,RideFilter
from services.rides import RideService,Optional, List, Dict
...
async def filter_rides_by_criteria(filter: RideFilter,  db: AsyncSession = Depends(get_db1)):
    try:
        rides = await RideService.filter_rides(db, filter)
